...
function Course({
  title, price, courseDuration, uploadedBy, status, thumbnailPath, rating,
  reviewCount, courseId, onRemoveFromCart, onRemoveFromBookmark, bookmarkId,
  isOpen, onEditCourse, onDeleteCourse, onDropdownClick,
}) 
{
  ...
  const fetchEnrollmentStatus = useCallback(async () => {
    try {
      ...
      const enrolledCourseIds = response.data.map(
        (enrolledCourse) => enrolledCourse.course.id
      );
  ...
  const fetchCartData = useCallback(async () => {
    try {
      ...
      const cartCourseIds = response.data.map(
        (cartCourse) => cartCourse.course.id
      );
  ...
  useEffect(() => {
    const isCourseBookmarked = localStorage.getItem(
      `bookmark_${localStorage.getItem("userId")}_${courseId}`
    );
  ...
  const handleClick = () => {
    const encryptedParams = CryptoJS.AES.encrypt(
      JSON.stringify({ status, enrolled: isEnrolled }),
      "secret"
    ).toString();
    navigate(
      `/courses/${courseId}?params=${encodeURIComponent(encryptedParams)}`
    );
  };
  ...
  const handleWishlistClick = async () => {
    try {
      ...
      if (isBookmarked) {
        ...
        handleAlert("Success", "Bookmark Removed", "success");
        ...
        handleAlert("Success", "Bookmark Added", "success");
      }
  ...
  const handleAddToCart = async () => {
    try {
      ...
      if (!isInCart) {
        ...
        setIsInCart(true);
        handleAlert("Success", "Course Added to Cart", "success");
      } else {
        navigate("/cart");
      }
    } catch (error) {
  ...
  const handleRemoveFromCart = async () => {
    try {
      ...
      setIsInCart(false);
      handleAlert("Success", "Course Removed from Cart", "success");
    ...
    >
      ...
      {userRole === "Teacher" && isMyCoursesPage && (
        <button
          ...
          onClick={() => onDropdownClick(courseId)}
        >
          <span title="">
            <BsThreeDotsVertical />
          </span>
        ...
        <div className="course-details">
          ...
          <div style={{ display: "flex" }} className="rating-review">
            <p style={{ color: "#ff4a12" }}>
              {rating}
              <span style={{ color: "#ff9413", fontSize: "19px" }}> ★ </span>
          ...
          {!isEnrolled && (
            <p className="p-r-i-c-e">
              {price === "0.00" ? "Free" : `Rs. ${price}`}
            </p>
          )}
